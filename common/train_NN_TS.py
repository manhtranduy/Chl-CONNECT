...
class train_NN:
    def __init__(self, 
        ...
        self.best_structure = self.arc_list[ind]
                    # Compare with the overall best model
                    # if score_train> best_score_train and score_val > best_score_val and score_val_noisy > best_score_val_noisy:
                    # if score_val > self.scoreThreshold and score_val_noisy > self.best_score_val_noisy:
                    # # if score_val_noisy > best_score_val_noisy:
                    #     self.best_score_train = score_train
                    #     self.best_score_val = score_val
                    #     self.best_score_val_noisy = score_val_noisy
                    #     self.best_model = model
                    #     self.best_structure = arc
...
def create_model(n_layers,
    ...
    for n_layer in range(n_layers):
        model.add(Dense(current_neurons, activation=activation, kernel_regularizer=tf.keras.regularizers.l2(l2_reg)))
        # model.add(BatchNormalization())  # Add BatchNormalization layer
        arc += (current_neurons,)
        current_neurons = int(current_neurons / n_factor)
        # current_neurons=5 * round(current_neurons / 5)
...
def train_model(X_train,Y_train,X_val,Y_val,X_val_noisy,
    ...
    for attempt in range(n_iter):  # Train 3 times for each architecture
        ...
        # history = model.fit(X_train, Y_train, validation_data=(X_val, Y_val), 
        #                     epochs=200, 
        #                     batch_size=200,  # Consider tuning
        #                     verbose=0)
